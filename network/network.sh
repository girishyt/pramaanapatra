...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "cryptogen" ]; then
    ...
    echo "##########################################################"
    echo "############ Create PUC Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-puc.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    echo "##########################################################"
    echo "############ Create ENG Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-eng.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo $'\e[1;32m'"Failed to generate certificates..."$'\e[0m'
    ...
    echo "##########################################################"
    echo "############ Create Training Identities ######################"
    echo "##########################################################"

    set -x
    cryptogen generate --config=./organizations/cryptogen/crypto-config-training.yaml --output="organizations"
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo $'\e[1;32m'"Failed to generate certificates..."$'\e[0m'
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/puc/msp organizations/fabric-ca/puc/tls-cert.pem organizations/fabric-ca/puc/ca-cert.pem organizations/fabric-ca/puc/IssuerPublicKey organizations/fabric-ca/puc/IssuerRevocationPublicKey organizations/fabric-ca/puc/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/eng/msp organizations/fabric-ca/eng/tls-cert.pem organizations/fabric-ca/eng/ca-cert.pem organizations/fabric-ca/eng/IssuerPublicKey organizations/fabric-ca/eng/IssuerRevocationPublicKey organizations/fabric-ca/eng/fabric-ca-server.db'
    docker run --rm -v $(pwd):/data busybox sh -c 'cd /data && rm -rf organizations/fabric-ca/training/msp organizations/fabric-ca/training/tls-cert.pem organizations/fabric-ca/training/ca-cert.pem organizations/fabric-ca/training/IssuerPublicKey organizations/fabric-ca/training/IssuerRevocationPublicKey organizations/fabric-ca/training/fabric-ca-server.db'
